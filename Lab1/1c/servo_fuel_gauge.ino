...
int servoPin1 = 23;
int servoPin2 = 32;      // GPIO pin used to connect the servo control (digital out)
...
void loop() {
  ...
  if (val < 60) {
    val2 = map(val, 0, 60, 0, 75); // Fast at the start
  } else if (val < 120) {
    val2 = map(val, 60, 120, 75, 80); // Slow in the middle
  } else {
    val2 = map(val, 120, 174, 80, 180); // Fast at the end
  }
